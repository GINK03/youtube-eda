    ...
    for pt in [pt]:
        try:
            ...
            if soup.find('a', {'id': 'youtube-user-page-country'}) is None:
                continue
            country = soup.find('a', {'id': 'youtube-user-page-country'}).text
            subs = soup.find('span', {'id': 'youtube-stats-header-subs'}).text
            views = soup.find(
                'span', {'id': 'youtube-stats-header-views'}).text
            uploads = soup.find(
                'span', {'id': 'youtube-stats-header-uploads'}).text
            ...
            for d1 in soup.find_all('div', {'style': 'float: left; width: 900px; height: 100px; margin-bottom: 10px;'}):
                for d2 in d1.find_all('div'):
                    if 'Estimated Monthly Earnings' in re.sub(r'\s{1,}', ' ', d2.text.strip()):
                        # print(d2.text.strip())
                        ma = re.search(
                            r'\$(\d|\.|K|M){1,} - \$(\d|\.|K|M){1,}', d2.text.strip())
                        estimate = ma.group(0)
...
df.to_csv('vars/channels.csv', index=None)
